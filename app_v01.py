...
def main():
    ...
    with tabs[4]:
        ...
        **IRR**, and **NPV** for a single scenario or a small range of scenarios.
        ...
        # If IRR not available, we store np.nan so styling won't fail
        ...
        # Convert IRR to float or np.nan to avoid styling error
        if isinstance(irr_value, float):
            irr_for_df = irr_value
        elif isinstance(irr_value, int):
            irr_for_df = float(irr_value)
        else:
            # IRR is None or some string -> store np.nan
            irr_for_df = np.nan
        ...
        # Use st.table with style.format to format all numeric columns
        st.table(df_cost.style.format("{:,.2f}"))

        # Show a note if IRR isn't available
        if IRR_AVAILABLE:
        ...
        if npv_project > 0:
            ...
            st.warning(f"Negative NPV => project might not be viable at these parameters. IRR={irr_for_df if not np.isnan(irr_for_df) else 'N/A'}")
