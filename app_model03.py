...
ENHANCED_INPUT_DATA = {
    "facility": {
        ...
        "gasification_temp_c": 700,  # Literature: e.g., Smith et al. (2018)
        # Updated from 75 to 91.5 kWh/ton MSW based on Kumar et al. (2021)
        "energy_consumption_kwh_per_ton": 91.5,
        # Updated from 800 to 550 kWh/ton MSW (typical waste-to-energy yield)
        ...
        # Optionally, you could add "msw_to_rdf_electricity_kwh_per_ton": 50 here if needed
    ...
    "ghg_baseline": {
        # Updated from 500 to 1721 kgCO2/ton (Kumar et al, 2021)
        "landfill_emissions_kg_per_ton": 1721,
        # Updated from 200 to 566.5 kgCO2/ton (Kumar et al, 2021)
        "facility_emissions_kg_per_ton": 566.5,
        "methane_factor_kg_co2eq_per_ton": 100
    },
    "trucking": {
        # Updated truck capacity from 36.0 to 15 ton (consulting input for an urban dump truck)
        "truck_capacity_tons": 15,
        # Updated distance from 200 km to 400 km (two-way trip)
        "distance_to_landfill_km": 400,
        # Updated emission factor from 0.8 to 2.26 kgCO2e/km (consulting input)
        "emission_factor_kgco2_per_km": 2.26
    }
}

# Multi-feedstock dictionary (updated to reflect a uniform electricity yield and facility emissions)
FEEDSTOCKS = {
    "RDF": {
        "fraction": 0.70,
...
def compute_bau_ghg_tons(daily_capacity, input_data):
    ...
    # Since methane capture is mandatory in Israel, we do not include the methane factor here.
...
# ---------------------------
def main():
    # Define tabs (merged scenario analysis into one tab)
    tabs = st.tabs([
        "Home",
        "Base Input Data",
        "Scenario Analysis",
    ...
    ###########################################################################
    with tabs[0]:
        # Use Streamlit columns to position title and logo
...
""")
    ...
    with tabs[1]:
        ...
        # Allow the client to update the truck capacity via the dashboard
        truck_capacity_input = st.number_input("Truck Capacity (ton)", min_value=5, max_value=50, 
                                                 value=ENHANCED_INPUT_DATA["trucking"]["truck_capacity_tons"], 
                                                 step=1, key="truck_capacity")
        ENHANCED_INPUT_DATA["trucking"]["truck_capacity_tons"] = truck_capacity_input
        
        show_footer()
    
    ###########################################################################
    # TAB 2: SCENARIO ANALYSIS (MERGED SINGLE & MULTI)
    ###########################################################################
    with tabs[2]:
            ...
            if st.button("Run Multi-Scenario", key="run_multi"):
                ...
                for cap in range(min_cap, max_cap+1, step_cap):
                    for cp in np.arange(min_c, max_c+0.001, step_c):
                        ...
                        rev_dict_ = compute_revenue_pie(cap, FEEDSTOCKS, ENHANCED_INPUT_DATA)
    ...
    with tabs[3]:
        ...
        if st.button("Run Year-by-Year Analysis", key="run_yby"):
            ...
            for year in sorted(YEARLY_FEEDSTOCK_DATA.keys()):
                ...
                revs_ = compute_revenue_pie(daily_tons, FEEDSTOCKS, ENHANCED_INPUT_DATA)
    ...
    with tabs[4]:
        ...
        if st.button("Run Monte Carlo", key="run_mc"):
            ...
            for i in range(int(runs_)):
                ...
                red_ = max(0, ghg_bau_ - ghg_proj_)
                revs_ = compute_revenue_pie(daily_cap, FEEDSTOCKS, ENHANCED_INPUT_DATA)
...
- Detailed scenario analysis (single and multi-scenario options).
